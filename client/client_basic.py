...
def receive(sock, signal, multi_list, stat_int):
    while signal:
        # try:
            # data = socket.recv(32)
...
receiveThread = threading.Thread(target = receive, args = (sock, True, multi_list, stat_int))
receiveThread.start()
#inputThread = threading.Thread(target=input_client)
#inputThread.start()
